...
/*
let res = odb.let('v', function(v) {
    v.create('VERTEX', 'Folder').set({name: 'root', id: uuid.v4()})
});

for (let i = 0; i < 3; i++) {
    res = res.let(`v${i}`, function(v) {
        v.create('VERTEX', 'Folder').set({name: 'testingjeje', id: uuid.v4()})
    })
}
res.commit().return('$v0').one().then(function(insertedItem) {
    console.log('jejeje');
    return Promise.resolve(insertedItem)
}).catch(function(err) {
    console.log(err);
