...
var folderService = {
     ...
     *
     * TODO: This function should be refactored. Remove "pyramid of doom".
     */
    findByPath(curriculumId, itemPath, expandParams) {
        ...
        return odb.query(query, queryParams).then(function(item) {
            if (!_.isEmpty(item)) {
                let itemResult = item[0];

                if (itemResult['@class'] === 'LearningObject') {
                    if (itemResult.deleted) {
                        return fs._removeLearningObjectById(itemResult.id).then(function() {
                            return Promise.resolve({})
                        })
                    } else if (itemResult.updated) {
                        return fs._syncLearningObject(itemResult)
                    } else {
            ...
            if (itemResult['@class'] === 'Folder') {
                ...
                if (_.includes(expandParams, 'metadata')) {
                    expandPromises.push(fs._getLearningObject(itemResult.id, itemResult.url));
            ...
            return Promise.all(expandPromises).then(function(expQueryresults) {
                _.each(expQueryresults, function(expQueryResult, i) {
                    if (expandProperties[i] === 'metadata') {
                        expQueryResult = expQueryResult.metadata;
                    }
    ...
    /**
     * Updates learningObject attributes when the 'updated' flag is true. It retrieves the original LO from cl-lo first.
     *
     * @param lo - object (LearningObject)
     * @returns Promise<Object> - The updated learning object.
     * @private
     */
    _syncLearningObject(lo) {
        let fs = this;
        let newLo = {};
        return fs._getLearningObject(lo.id, lo.url).then(function(updatedLo) {
            if (_.isEmpty(updatedLo)) {
                log.info(`[FolderService][_syncLearningObject] Could not sync learning object with id='${lo.id}' and url='${lo.url}'\n`);
                // The outdated learningObject will be returned, keeping the updated flag on.
                return Promise.resolve({updatedRecords: 0})
            }

            lo.updated = newLo.updated = false;
            lo.title = newLo.title = updatedLo.title;
            newLo.url = lo.url;

            let learningObjectives = [];
            _.forEach(updatedLo.learningObjectiveList, function(learningObjective) {
                learningObjectives.push({
                    name: learningObjective.name,
                    description: learningObjective.description,
                    url: `/learningObjectives/${learningObjective.id}`
                })
            });
            lo.learningObjectives = newLo.learningObjectives = learningObjectives;

            // Update the cl-curricula lo copy.
            return fs._updateLearningObjectById(lo.id, newLo)
        }).then(function(res) {
    ...
    /**
     * Delete the specified learning object vertex from database.
     *
     * @param id - string
     * @returns Promise<object> - The object contains the number of deleted records.
     */
    _removeLearningObjectById(id) {
    ...
    /**
     * Updates the specified learning object vertex.
     *
     * @param id - string
     * @param newLo - object
     * @returns Promise<object> - The object contains the number of updated records.
     */
    _updateLearningObjectById(id, newLo) {
        _addEmbeddedProperties(newLo, 'LearningObject');
        let queryParams = {params: {content: newLo, id}};
        let query = `UPDATE LearningObject MERGE :content WHERE id = :id`;
    ...
    _findLearningObjects(folderId) {
        let queryParams = {params: {id: folderId}};
        let query = `SELECT id, name, title, url, contentUrl, learningObjectives, learningObjectId 
    ...
    /**
     * Makes a GET request to cl-lo to obtain a specific learningObject.
     *
     * @param loId - string
     * @param loURL - string (url referencing the learningObject in cl-lo)
     * @returns Promise<object> - learningObject.
     * @private
     */
    _getLearningObject(loId, loURL) {
        return LearningObject.client.get(loURL).then(function(res) {
            return Promise.resolve(res)
        }).catch(function(err) {
            log.info(`[FolderService][getLearningObject] Could not get learning object with id='${loId}' and url='${loURL}'\n`, err);
            return Promise.resolve({})
        })
    },
    ...
    _saveItem(curriculumId, parentFolder, newItem) {
        ...
        // If newItem is a LearningObject, set the sync flags to false.
        if (itemClass === 'LearningObject') {
        ...
        return this._containsItem(parentFolder.id, newItem.name).then(function(exists) {
            ...
            return odb.let('item', function(c) {
                ...
                log.info(`${itemClass} with id: ${newItem.id} was successfully created.`);
