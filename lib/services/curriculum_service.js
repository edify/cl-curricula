...
var curriculumService = {
     ...
     * Retrieves all curricula from database that are related with a learningObjective.
     *
     * @param curriculumId - String   (Curriculum unique identifier).
     *
     * @returns {Promise<Array<LearningObjectives>>}. Custom error otherwise.
     */
    getLinkedLearningObjectives(curriculumId) {
        let queryParams = {params: {id: curriculumId}};
        let query = `SELECT learningObjectives FROM 
                            (TRAVERSE OUT() FROM 
                                (SELECT FROM Curriculum WHERE id = :id)) 
                     WHERE @class= 'LearningObject'`;
        return odb.query(query, queryParams).then(function (results) {
            results = _.map(results, function(it) { return it.learningObjectives });
            let learningObjectives = _.each(_.flattenDeep(results), objectReducer.reduce);
            let uniqueLearningObjectives = _.uniqWith(learningObjectives, _.isEqual);
            return Promise.resolve(uniqueLearningObjectives)
        }).catch(function(err) {
            return Promise.reject(err)
        })
    }
